...
contains
  ...
  subroutine esm_time_clockInit(ensemble_driver, instance_driver, logunit, maintask, inComp, rc)

    ! input/output variables
    type(ESMF_GridComp)  :: ensemble_driver, instance_driver
    integer, intent(in)  :: logunit
    logical, intent(in)  :: maintask
    logical, intent(in) :: inComp
    ...
    logical                 :: isPresent
    logical, save           :: ensemble_clock_initialized = .false. 
    ...
    if (read_restart) then
       ...
       call NUOPC_CompAttributeGet(ensemble_driver, name='drv_restart_pointer', value=restart_file, rc=rc)
       if (ChkErr(rc,__LINE__,u_FILE_u)) return

       if (inComp .and. trim(restart_file) /= 'none') then
    ...
    call ESMF_VMBroadcast(ensemble_vm, tmp, 4, 0, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    ...
    !---------------------------------------------------------------------------
    if(.not. ensemble_clock_initialized) then
       ...
       call ESMF_GridCompSet(ensemble_driver, clock=clock, rc=rc)
       if (ChkErr(rc,__LINE__,u_FILE_u)) return
       ensemble_clock_initialized = .true.
    endif
